@with_kw mutable struct FSM{Tfloat, Tint}
  ...
  # State variables
  albs::Array{Tfloat, 2} = Tfloat(0.85)*ones(Nx,Ny)
  Ds::Array{Tfloat,3} = zeros(Nsmax,Nx,Ny)
  Nsnow::Array{Tint,2} = zeros(Tint,Nx,Ny)
  ...
  Tsnow::Array{Tfloat,3} = Tfloat(273.15)*ones(Nsmax,Nx,Ny)
  Tsoil::Array{Tfloat,3} = Tfloat(285)*ones(Nsoil,Nx,Ny)
  Tsrf::Array{Tfloat, 2} = Tfloat(285)*ones(Nx,Ny)
