function snow_layering!(fsm::FSM{Tf, Ti}, meteo::MET{Tf, Ti}, snowdepth0, Sice0) where {Tf<:Real, Ti<:Integer}
    ...
    for j in 1:Ny
        for i in 1:Nx
            ...
            if tilefrac[i,j] >= tthresh
                ...
                snowcoverfraction!(fsm, snowdepth, SWEtmp, i, j)
                ...
                if SNOLAY == 0
                    ...
                    S[:] = @view Sice[:,i,j]
                    W[:] = @view Sliq[:,i,j]
                    hw[:] = @view histowet[:,i,j]
                    if fsnow[i,j] > eps(Tf)
                        csnow[1] = (Sice[1,i,j]*hcap_ice + Sliq[1,i,j]*hcap_wat) / fsnow[i,j]
                        E[1] = csnow[1]*(Tsnow[1,i,j] - Tm) + (Sice0[i,j] * hcap_ice / fsnow[i,j]) * (Tsnow0 - Tsnow[1,i,j])
                    else
                        E[:] .= 0
                    end
                    if Nsnow[i,j] > 1
                    ...
                    if Ds0[i,j] > eps(Tf)
                        snowthickness = Ds0[i,j] + sum(@view Ds[:,i,j])
                    else
                        snowthickness = sum(@view Ds[:,i,j])
                    end
                    rho[:] .= rho0
                    
                    # Step 0: Save state variables in local variables that can be up to Nsmax+1
                    Sice_loc[1:Nsmax] = @view Sice[:,i,j]
                    Sice_loc[Nsmax+1] = 0
                    Sliq_loc[1:Nsmax] = @view Sliq[:,i,j]
                    Sliq_loc[Nsmax+1] = 0
                    Ds_loc[1:Nsmax] = @view Ds[:,i,j]
                    Ds_loc[Nsmax+1] = 0
                    histowet_loc[1:Nsmax] = @view histowet[:,i,j]
                    histowet_loc[Nsmax+1] = 0
                    Tsnow_loc[1:Nsmax] = @view Tsnow[:,i,j]
                    ...
                    if snowthickness >= eps(Tf)
                        ...
                        if Nsnow_loc > 1
                            ...
                            if k_surflay > 0
                                ...
                                histowet_loc[k_surflay+1] = (sum(@view((Sice_loc[k_surflay+1:Nsnow_loc] .+ Sliq_loc[k_surflay+1:Nsnow_loc]) .* histowet_loc[k_surflay+1:Nsnow_loc])) +
                                                           (Sice_old + Sliq_old) * (1 - Ds_loc[k_surflay] / Ds_old) * histowet_loc[k_surflay]) /
                                                           (sum(@view(Sice_loc[k_surflay+1:Nsnow_loc] .+ Sliq_loc[k_surflay+1:Nsnow_loc])) +
                                                           (Sice_old + Sliq_old) * (1 - Ds_loc[k_surflay] / Ds_old))
                                Sice_loc[k_surflay+1] = sum(@view Sice_loc[k_surflay+1:Nsnow_loc]) + Sice_old * (1 - Ds_loc[k_surflay] / Ds_old)
                                Sliq_loc[k_surflay+1] = sum(@view Sliq_loc[k_surflay+1:Nsnow_loc]) + Sliq_old * (1 - Ds_loc[k_surflay] / Ds_old)
                                U_loc[k_surflay+1] = sum(@view U_loc[k_surflay+1:Nsnow_loc]) + U_old * (1 - Ds_loc[k_surflay] / Ds_old)
                        ...
                        while minimum(@view Ds_loc[1:Nsnow_loc]) < Ds_min && Nsnow_loc > 1
                            # Find the thinnest layer
                            kmin = argmin(@view Ds_loc[1:Nsnow_loc])
                            if kmin == 1
                        ...
                        while Nsnow_loc > Nsmax
                            ...
                            rho[1:Nsnow_loc] = (@view(Sice_loc[1:Nsnow_loc]) .+ @view(Sliq_loc[1:Nsnow_loc])) ./ @view(Ds_loc[1:Nsnow_loc]) ./ fsnow[i,j]
                            for k in 1:(Nsnow_loc - 1)
                            ...
                            kmerge = argmin(@view diff_rho[1:(Nsnow_loc-1)])
                        ...
                        if Nsnow_loc > 0
                            while Nsnow_loc < Nsmax
                                if Nsnow_loc == 1
                                    ...
                                    if Ds_loc[kmax] < 2.0 * Ds_min
                                        break
                                    end
                                    if Ds_loc[kmax] / 2.0 <= Ds_surflay
                                        # We can split the bottom layer in two
                                        # and the new surface layer will be thinner than Ds_surflay.
                                        Nsnow_loc = Nsnow_loc + 1
                                        Ds_loc[Nsnow_loc] = Ds_loc[kmax] / 2.0
                                        Ds_loc[kmax] = Ds_loc[kmax] / 2.0
                                        Sice_loc[Nsnow_loc] = Sice_loc[kmax] / 2.0
                                        Sice_loc[kmax] = Sice_loc[kmax] / 2.0
                                        Sliq_loc[Nsnow_loc] = Sliq_loc[kmax] / 2.0
                                        Sliq_loc[kmax] = Sliq_loc[kmax] / 2.0
                                        histowet_loc[Nsnow_loc] = histowet_loc[kmax]
                                        U_loc[Nsnow_loc] = U_loc[kmax] / 2.0
                                        U_loc[kmax] = U_loc[kmax] / 2.0
                                    else
                                        ...
                                        Ds_loc[Nsnow_loc] = (1.0 - wt) * Ds_loc[kmax]
                                        Ds_loc[kmax] = wt * Ds_loc[kmax]
                                        Sice_loc[Nsnow_loc] = (1.0 - wt) * Sice_loc[kmax]
                                        Sice_loc[kmax] = wt * Sice_loc[kmax]
                                        Sliq_loc[Nsnow_loc] = (1.0 - wt) * Sliq_loc[kmax]
                                        Sliq_loc[kmax] = wt * Sliq_loc[kmax]
                                        histowet_loc[Nsnow_loc] = histowet_loc[kmax]
                                        U_loc[Nsnow_loc] = (1.0 - wt) * U_loc[kmax]
                                        U_loc[kmax] = wt * U_loc[kmax]
                                    end
                                else
                                    # More than one layer
                                    # Find the thickest layer
                                    kmax = argmax(@view Ds_loc[1:Nsnow_loc])
                                    # If the thickest layer is too thin to split, stop
                                    if Ds_loc[kmax] < 2.0 * Ds_min
                                        break
                                    end
                                    Dtemp_surflay = sum(@view Ds_loc[1:(Nsnow_loc-1)])
                                    if (kmax == Nsnow_loc) && (Ds_surflay - Dtemp_surflay > Ds_min)
                                        ...
                                        kmax = argmax(@view Ds_loc[1:(Nsnow_loc-1)])
                                        # If the thickest layer is too thin to split, stop
                                        if Ds_loc[kmax] < 2.0 * Ds_min
                                            break
                                        end
                                        ...
                                        Ds_loc[kmax+1] = Ds_loc[kmax] / 2.0
                                        Ds_loc[kmax] = Ds_loc[kmax] / 2.0
                                        Sice_loc[kmax+1] = Sice_loc[kmax] / 2.0
                                        Sice_loc[kmax] = Sice_loc[kmax] / 2.0
                                        Sliq_loc[kmax+1] = Sliq_loc[kmax] / 2.0
                                        Sliq_loc[kmax] = Sliq_loc[kmax] / 2.0
                                        histowet_loc[kmax+1] = histowet_loc[kmax]
                                        U_loc[kmax+1] = U_loc[kmax] / 2.0
                                        U_loc[kmax] = U_loc[kmax] / 2.0
                                    end
                                end
                            end
                    ...
                    @view(Sice[:,i,j]) .= @view Sice_loc[1:Nsmax]
                    @view(Sliq[:,i,j]) .= @view Sliq_loc[1:Nsmax]
                    @view(Ds[:,i,j]) .= @view Ds_loc[1:Nsmax]
                    @view(histowet[:,i,j]) .= @view histowet_loc[1:Nsmax]
                    @view(Tsnow[:,i,j]) .= @view Tsnow_loc[1:Nsmax]
                    Nsnow[i,j] = Nsnow_loc
