...
! SNOLAY = 0 is the original FSM2 fixed layering (Essery, 2015)
! SNOLAY = 1 is the density-dependent layering 
! developped and implemented by LQ (Quéno et al., 2024) 
!-----------------------------------------------------------------------
subroutine SNOW_LAYERING(snowdepth0,Sice0)

use MODCONF, only: HN_ON, SNOLAY
...
do j = 1, Ny
  do i = 1, Nx
    ...
    if (tilefrac(i,j) >= tthresh) then ! exclude points outside tile of interest

      ! Decrease Nsnow if necessary (e.g. after melting)
      do while (Nsnow(i,j) > 0 .and. Ds(1,i,j) < epsilon(Ds(1,i,j)))
        if (Nsnow(i,j) > 1) then
          do n = 1 , Nsnow(i,j) - 1
            Ds(n,i,j) = Ds(n+1,i,j)
      ...
      if (SNOLAY == 0) then
        ...
        if (snowthickness >= epsilon(snowthickness)) then
          ...
          do while (minval(Ds_loc(1:Nsnow_loc)) < Ds_min .and. Nsnow_loc > 1)
          ...
          if (Nsnow_loc > 0) then
            do while(Nsnow_loc < Nsmax)
              if (Nsnow_loc == 1) then
                ...
                if (Ds_loc(kmax) < 2.0 * Ds_min) exit
                if (Ds_loc(kmax) / 2.0 <= Ds_surflay) then
                ...
                if ((kmax == Nsnow_loc) .and. (Ds_surflay - Dtemp_surflay > Ds_min)) then
                  ...
                  ! Calculate the thickness of the surface layers
                    if (Ds_loc(kmax) - (Ds_surflay - Dtemp_surflay) > Ds_min) then
